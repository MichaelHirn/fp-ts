...
export const getOrElseW: <B>(
  onNone: Lazy<Task<B>>,
) => <A>(ma: TaskOption<A>) => Task<A | B> = getOrElse as any;
 ...
 */
export const alt: <A>(
  second: Lazy<TaskOption<A>>,
) => (first: TaskOption<A>) => TaskOption<A> =
  /*#__PURE__*/
  OT.alt(T.Monad);
...
) => <A>(first: TaskOption<A>) => TaskOption<A | B> = alt as any;
...
export const filter: {
  ...
  ): (fb: TaskOption<A>) => TaskOption<B>;
  <A>(
    predicate: Predicate<A>,
  ): <B extends A>(fb: TaskOption<B>) => TaskOption<B>;
...
export const partition: {
  <A, B extends A>(
    refinement: Refinement<A, B>,
  ): (fb: TaskOption<A>) => Separated<TaskOption<A>, TaskOption<B>>;
  <A>(
    predicate: Predicate<A>,
  ): <B extends A>(
    fb: TaskOption<B>,
  ) => Separated<TaskOption<B>, TaskOption<B>>;
  <A>(
    predicate: Predicate<A>,
  ): (fa: TaskOption<A>) => Separated<TaskOption<A>, TaskOption<A>>;
} =
  /*#__PURE__*/
...
export const traverseReadonlyNonEmptyArrayWithIndex = <A, B>(
  ...
  flow(
    T.traverseReadonlyNonEmptyArrayWithIndex(f),
    T.map(O.traverseReadonlyNonEmptyArrayWithIndex(SK)),
  );
 ...
 * @since 2.11.0
 */
export const traverseReadonlyNonEmptyArrayWithIndexSeq = <A, B>(
  f: (index: number, a: A) => TaskOption<B>,
) =>
  (
    ...
    () =>
      _.tail(as).reduce<Promise<Option<NonEmptyArray<B>>>>(
        (acc, a, i) =>
          acc.then((obs) =>
            _.isNone(obs) ? acc : f(i + 1, a)().then((ob) => {
              if (_.isNone(ob)) {
                return ob;
              }
...
export const traverseArray: <A, B>(
  f: (a: A) => TaskOption<B>,
) => (as: ReadonlyArray<A>) => TaskOption<ReadonlyArray<B>> = (f) =>
