...
export const getOrElseW: <E, B>(
  onLeft: (e: E) => IO<B>,
) => <A>(ma: IOEither<E, A>) => IO<A | B> = getOrElse as any;
...
): IOEither<E, A> => () => E.tryCatch(f, onThrow);
...
) => (ma: IOEither<E1, A>) => IOEither<E2, A> =
  /*#__PURE__*/
  ET.orElse(I.Monad);
...
export const orElseFirstIOK: <E, B>(
  onLeft: (e: E) => IO<B>,
) => <A>(ma: IOEither<E, A>) => IOEither<E, A> = (onLeft) =>
...
export const orLeft: <E1, E2>(
  onLeft: (e: E1) => IO<E2>,
) => <A>(fa: IOEither<E1, A>) => IOEither<E2, A> =
  /*#__PURE__*/
  ET.orLeft(I.Monad);
...
export const map: <A, B>(
  f: (a: A) => B,
) => <E>(fa: IOEither<E, A>) => IOEither<E, B> =
  /*#__PURE__*/
  ET.map(I.Functor);
...
export const mapLeft: <E, G>(
  f: (e: E) => G,
) => <A>(fa: IOEither<E, A>) => IOEither<G, A> =
  /*#__PURE__*/
...
export const flatten: <E, A>(
  mma: IOEither<E, IOEither<E, A>>,
) => IOEither<E, A> = flattenW;
 ...
 * @since 2.7.0
 */
export function getApplicativeIOValidation<E>(
  S: Semigroup<E>,
): Applicative2C<URI, E> {
...
export function getFilterable<E>(M: Monoid<E>): Filterable2C<URI, E> {
  ...
  return {
    ...
    compact: C.compact,
    separate: C.separate,
    filter: <A>(fa: IOEither<E, A>, predicate: Predicate<A>) =>
...
export const filterOrElse: {
  ...
  <E, A>(
    predicate: Predicate<A>,
    onFalse: (a: A) => E,
  ): <B extends A>(mb: IOEither<E, B>) => IOEither<E, B>;
  <E, A>(
    predicate: Predicate<A>,
    onFalse: (a: A) => E,
  ): (ma: IOEither<E, A>) => IOEither<E, A>;
...
export const traverseReadonlyNonEmptyArrayWithIndex: <A, E, B>(
  ...
  flow(
    I.traverseReadonlyNonEmptyArrayWithIndex(f),
    I.map(E.traverseReadonlyNonEmptyArrayWithIndex(SK)),
  );
 ...
 * @since 2.11.0
 */
export const traverseReadonlyNonEmptyArrayWithIndexSeq = <A, E, B>(
  f: (index: number, a: A) => IOEither<E, B>,
) =>
  (
...
export const getSemigroup = <E, A>(
  S: Semigroup<A>,
): Semigroup<IOEither<E, A>> => getApplySemigroup_(I.Apply)(E.getSemigroup(S));
