...
export const local: <R2, R1>(
  f: (r2: R2) => R1,
) => <A>(ma: ReaderTask<R1, A>) => ReaderTask<R2, A> = R.local;
 ...
 * @since 2.11.0
 */
export const asksReaderTaskW: <R1, R2, A>(
  f: (r1: R1) => ReaderTask<R2, A>,
) => ReaderTask<R1 & R2, A> = R.asksReaderW;
 ...
 * @since 2.11.0
 */
export const asksReaderTask: <R, A>(
  f: (r: R) => ReaderTask<R, A>,
) => ReaderTask<R, A> = asksReaderTaskW;
...
export const map: <A, B>(
  f: (a: A) => B,
) => <R>(fa: ReaderTask<R, A>) => ReaderTask<R, B> =
  /*#__PURE__*/
  RT.map(T.Functor);
 ...
 * @since 2.3.0
 */
export const flatten: <R, A>(
  mma: ReaderTask<R, ReaderTask<R, A>>,
) => ReaderTask<R, A> = flattenW;
...
) => ReaderTask<R, ReadonlyNonEmptyArray<B>>) =>
  flow(
    R.traverseReadonlyNonEmptyArrayWithIndex(f),
    R.map(T.traverseReadonlyNonEmptyArrayWithIndex(SK)),
  );
...
export const traverseReadonlyNonEmptyArrayWithIndexSeq = <R, A, B>(
  ...
  flow(
    R.traverseReadonlyNonEmptyArrayWithIndex(f),
    R.map(T.traverseReadonlyNonEmptyArrayWithIndexSeq(SK)),
  );
 ...
 * @deprecated
 */
export const getSemigroup: <R, A>(
  S: Semigroup<A>,
) => Semigroup<ReaderTask<R, A>> =
  /*#__PURE__*/
