...
export const local: <R2, R1>(
  f: (r2: R2) => R1,
) => <A>(ma: Reader<R1, A>) => Reader<R2, A> = (f) => (ma) => (r2) => ma(f(r2));
 ...
 * @since 2.11.0
 */
export const asksReaderW = <R1, R2, A>(
  f: (r1: R1) => Reader<R2, A>,
): Reader<R1 & R2, A> => (r) => f(r)(r);
 ...
 * @since 2.11.0
 */
export const asksReader: <R, A>(f: (r: R) => Reader<R, A>) => Reader<R, A> =
  asksReaderW;
...
export const map: <A, B>(
  f: (a: A) => B,
) => <R>(fa: Reader<R, A>) => Reader<R, B> = (f) => (fa) => (r) => f(fa(r));
...
export const apW: <R2, A>(
  fa: Reader<R2, A>,
) => <R1, B>(fab: Reader<R1, (a: A) => B>) => Reader<R1 & R2, B> = (fa) =>
  (
    fab,
  ) => (r) => fab(r)(fa(r));
...
export const ap: <R, A>(
  fa: Reader<R, A>,
) => <B>(fab: Reader<R, (a: A) => B>) => Reader<R, B> = apW;
...
export const chainW: <R2, A, B>(
  f: (a: A) => Reader<R2, B>,
) => <R1>(ma: Reader<R1, A>) => Reader<R1 & R2, B> = (f) =>
  (
    fa,
  ) => (r) => f(fa(r))(r);
...
export const chain: <A, R, B>(
  f: (a: A) => Reader<R, B>,
) => (ma: Reader<R, A>) => Reader<R, B> = chainW;
...
export const compose: <A, B>(
  ab: Reader<A, B>,
) => <C>(bc: Reader<B, C>) => Reader<A, C> = (ab) => (bc) => flow(ab, bc);

/**
 * @category Profunctor
 * @since 2.0.0
 */
export const promap: <E, A, D, B>(
  f: (d: D) => E,
  g: (a: A) => B,
) => (fea: Reader<E, A>) => Reader<D, B> = (f, g) =>
  (
    fea,
  ) => (a) => g(fea(f(a)));
 ...
 * @since 2.10.0
 */
export const first: Strong2<URI>["first"] = <A, B, C>(pab: Reader<A, B>) =>
  ([a, c]: [A, C]) => [pab(a), c];
 ...
 * @since 2.10.0
 */
export const second: Strong2<URI>["second"] = <A, B, C>(pbc: Reader<B, C>) =>
  ([a, b]: [A, B]) => [a, pbc(b)];
 ...
 * @since 2.10.0
 */
export const left: Choice2<URI>["left"] = <A, B>(pab: Reader<A, B>) =>
  E.fold((a: A) => _.left(pab(a)), (b: B) => E.right(b));

/**
 * @category Choice
 * @since 2.10.0
 */
export const right: Choice2<URI>["right"] = (pbc) =>
  E.fold(E.left, (b) => _.right(pbc(b)));
...
export const traverseReadonlyNonEmptyArrayWithIndex = <A, R, B>(
  ...
  ): Reader<R, ReadonlyNonEmptyArray<B>> =>
    (r) => {
