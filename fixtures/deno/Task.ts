 ...
 * @since 2.0.0
 */
export const map: <A, B>(f: (a: A) => B) => (fa: Task<A>) => Task<B> = (f) =>
 ...
 * @since 2.0.0
 */
export const ap: <A>(fa: Task<A>) => <B>(fab: Task<(a: A) => B>) => Task<B> = (
  fa,
) =>
  (fab) =>
    () =>
      Promise.all([Promise.resolve().then(fab), Promise.resolve().then(fa)])
        .then(([f, a]) => f(a));
 ...
 * @since 2.0.0
 */
export const chain: <A, B>(f: (a: A) => Task<B>) => (ma: Task<A>) => Task<B> = (
  f,
) =>
  (ma) =>
    () =>
...
export function getRaceMonoid<A = never>(): Monoid<Task<A>> {
  return {
    concat: (x, y) =>
      () =>
        Promise.race([Promise.resolve().then(x), Promise.resolve().then(y)]),
    empty: never,
  };
}
...
export const traverseReadonlyNonEmptyArrayWithIndex = <A, B>(
  ...
  (
    ...
    () =>
      Promise.all(
        as.map((a, i) => Promise.resolve().then(() => f(i, a)())),
      ) as any;
