...
export const ap: <E, A>(
  fa: State<E, A>,
) => <B>(fab: State<E, (a: A) => B>) => State<E, B> = (fa) =>
  (fab) =>
    (s1) => {
      const [f, s2] = fab(s1);
      const [a, s3] = fa(s2);
 ...
 * @since 2.0.0
 */
export const of: Pointed2<URI>["of"] = <S, A>(a: A) => (s: S) => [a, s];
 ...
 * @since 2.11.0
 */
export const traverseReadonlyNonEmptyArrayWithIndex = <A, S, B>(
 ...
 * @deprecated
 */
export const evalState: <S, A>(ma: State<S, A>, s: S) => A = (ma, s) =>
  ma(s)[0];
 ...
 * @deprecated
 */
export const execState: <S, A>(ma: State<S, A>, s: S) => S = (ma, s) =>
  ma(s)[1];
