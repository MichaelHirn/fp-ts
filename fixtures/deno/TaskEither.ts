...
export const fromTaskOption: <E>(
  onNone: Lazy<E>,
) => NaturalTransformation12C<TOURI, URI, E> = (onNone) =>
...
export const chainNullableK: <E>(
  ...
  f: (a: A) => B | null | undefined,
) => (ma: TaskEither<E, A>) => TaskEither<E, NonNullable<B>> =
  /*#__PURE__*/
...
export const chainIOEitherKW: <E2, A, B>(
  f: (a: A) => IOEither<E2, B>,
) => <E1>(ma: TaskEither<E1, A>) => TaskEither<E1 | E2, B> = (f) =>
...
export const map: <A, B>(
  f: (a: A) => B,
) => <E>(fa: TaskEither<E, A>) => TaskEither<E, B> =
  /*#__PURE__*/
  ET.map(T.Functor);
...
export const flatten: <E, A>(
  mma: TaskEither<E, TaskEither<E, A>>,
) => TaskEither<E, A> = flattenW;
 ...
 * @since 2.7.0
 */
export function getApplicativeTaskValidation<E>(
  A: Apply1<T.URI>,
  S: Semigroup<E>,
): Applicative2C<URI, E> {
...
export function getFilterable<E>(M: Monoid<E>): Filterable2C<URI, E> {
  ...
  return {
    ...
    compact: C.compact,
    separate: C.separate,
    filter: <A>(fa: TaskEither<E, A>, predicate: Predicate<A>) =>
...
export const filterOrElse: {
  ...
  <E, A>(
    predicate: Predicate<A>,
    onFalse: (a: A) => E,
  ): <B extends A>(mb: TaskEither<E, B>) => TaskEither<E, B>;
  <E, A>(
    predicate: Predicate<A>,
    onFalse: (a: A) => E,
  ): (ma: TaskEither<E, A>) => TaskEither<E, A>;
    ...
    return () =>
      new Promise((resolve) => {
        const cbResolver = (
          e: L,
          r: R,
        ) => (e != null ? resolve(_.left(e)) : resolve(_.right(r)));
...
export const traverseReadonlyNonEmptyArrayWithIndex = <A, E, B>(
  ...
  flow(
    ...
    T.map(E.traverseReadonlyNonEmptyArrayWithIndex(SK)),
  );
  ...
  (
    ...
    () =>
      _.tail(as).reduce<Promise<Either<E, NonEmptyArray<B>>>>(
        (acc, a, i) =>
          acc.then((ebs) =>
            _.isLeft(ebs) ? acc : f(i + 1, a)().then((eb) => {
              if (_.isLeft(eb)) {
                return eb;
              }
